...
The generated Touchstone .sNp files have been tested using Python scikit-rf
library and ADS data file tool.
    ...
    def lines(self):
        ...
        if not hasattr(self, "_lines"):
            ...
            if os.path.exists(self.input_file):
                ...
                self._lines = []
                self.print_log(
                    type="E", msg=f"File {self.input_file} does not exist."
                )
        return self._lines
        ...
        while i < len(self.lines):
            ...
            # Skip empty lines
                ...
                if "CONSTANT" in parts[0]:
                    ...
                    parts[0] == "VAR"
                ):  # == used to distinguis from VAR_LIST_BEGIN
                    self.var_name, self.var_format, var_nbr_of_points = parts[
                        1
                    ].split(" ")
                    ...
                    if "MAG" in self.var_format:
                        self.var_data = np.loadtxt(
                            self.input_file,
                            dtype=float,
                            skiprows=i,
                            max_rows=self.var_nbr_of_points,
                        )
                    else:
                        self.print_log(
                            type="E",
                            msg=f"Frequency format: {self.var_format} should not exist. Check your CITIFile...",
                        )
                    ...
                    if "RI" in self.data[name]["format"]:
                        self.data[name]["data"] = np.loadtxt(
                            self.input_file,
                            dtype=float,
                            skiprows=i,
                            max_rows=self.var_nbr_of_points,
                        ...
                        self.print_log(
                            type="E",
                            msg=f"Data format: {self.data[name]['format']} should not exist. Check your CITIFile...",
                        )
