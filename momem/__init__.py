...
class momem(thesdk, metaclass=abc.ABCMeta):
    ...
    def has_lsf(self):
        ...
        """
        if (not thesdk.GLOBALS["LSFINTERACTIVE"] == "") and (
            not thesdk.GLOBALS["LSFINTERACTIVE"] == ""
        ):
            self._has_lsf = True
        else:
            self._has_lsf = False
    ...
    def momem_submission(self):
        ...
        if not hasattr(self, "_momem_submission"):
            try:
                if not self.has_lsf:
                    ...
                    if self.interactive_momem:
                        if not self.distributed_run:
                            ...
                            self._momem_submission = thesdk.GLOBALS[
                                "LSFSUBMISSION"
                            ] + " -o %s/bsublog.txt " % (self.momemsimpath)
                    else:
                        self._momem_submission = thesdk.GLOBALS[
                            "LSFSUBMISSION"
                        ] + " -o %s/bsublog.txt " % (self.momemsimpath)
                ...
                self.print_log(
                    type="W",
                    msg="Error while defining momem submission command. Running locally.",
                )
                self._momem_submission = ""
    ...
    def momemsrcpath(self):
        ...
        if not (os.path.exists(self._momemsrcpath)):
            self.print_log(
                type="E",
                msg="The %s source entity folder does not exist. Run configure!"
                % self._momemsrcpath,
            )
    ...
    def momemsimpath(self):
        ...
        if not hasattr(self, "_momemsimpath"):
            ...
            if os.path.exists(self._momemsimpath):
                self.print_log(
                    type="I", msg=f"Overwriting {self._momemsimpath}"
                )
                targetpath = f"{self._momemsimpath}"
                self.print_log(type="I", msg=f"Deleting {targetpath}")
                shutil.rmtree(targetpath)
            self.print_log(type="I", msg=f"Creating {self._momemsimpath}")
    ...
    def cleanup_momemsimpath(self):
        ...
        if os.path.exists(self.momemsimpath):
            self.print_log(type="I", msg=f"Cleaning up {self.momemsimpath}")
    ...
    def read_simulation_results(self):
        ...
        try:
            filename = glob.glob(
                f"{self.momemsimpath}/{self.result_filenames}.s*p"
            )[0]
            self.print_log(
                type="I", msg=f"Reading simulation results from {filename}."
            )
            self.extracts.Members["net"] = rf.Network(filename)
            self.print_log(
                type="I", msg=f"Saveed results to self.extracts.Members['net']."
            )
        except:
            self.print_log(type="W", msg=traceback.format_exc())
            self.print_log(
                type="W",
                msg=f"Something went wrong while reading simulation results from {filename}.",
            )
            filepath = self.momemsimpath.split(self.runname)[0]
            self.print_log(
                type="F", msg=f"Available results: {os.listdir(filepath)}"
            )
