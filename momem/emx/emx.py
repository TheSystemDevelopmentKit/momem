...
class emx(thesdk):
    ...
    **kwargs :
    ...
    def generate_gds(self):
        ...
        """
        self.print_log(type="I", msg=f"Running external command {self.gdscmd}")
    ...
    def emxcmd(self):
        ...
        for sim, val in self.parent.simcmd_bundle.Members.items():
            ...
            if val.sim == "sweep":
                if len(val.swpvalues) == 0 and any(
                    (
                        val.swpstart == None,
                        val.swpstop == None,
                        val.swpstep == None,
                ...
                self.print_log(
                    type="F", msg=f"Invalid simulation type for momem"
                )
    ...
    def execute_emx_simulation(self):
        ...
        try:
            self.print_log(
                type="I", msg=f"Running external command {self.emxcmd}"
            )
            subprocess.check_output(self.emxcmd, shell=True)
        except:
            self.print_log(type="E", msg=traceback.format_exc())
            self.print_log(type="F", msg="Running simulation failed.")
